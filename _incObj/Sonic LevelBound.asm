...
.bottom:
		...
		beq.s	.check_position           ; if it IS SBZ2, check position
		jmp	KillSonic                 ; if not, kill Sonic (long jump)

.check_position:
		cmpi.w	#$2000,(v_player+obX).w
		bhs.s	.dontkill                 ; if higher or equal, don't kill
		jmp	KillSonic                 ; if lower, kill Sonic (long jump)
		else
		blt.s	.dontkill	; if so, don't kill Sonic
		cmpi.w	#(id_SBZ<<8)+1,(v_zone).w ; is level SBZ2 ?
		if	AfterImageSonic=1
    beq.s .skipKill1 ; Inverte bne para beq (pula se for igual)
    jmp KillSonic    ; Usa jmp, que alcança qualquer lugar da memória
.skipKill1:
    bhs.s .skipKill2 ; Inverte blo para bhs (pula se for maior ou igual)
    jmp KillSonic    ; Usa jmp para alcançar a rotina distante
.skipKill2:
		else
