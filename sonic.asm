...
HPBossCount	=	8
;   | Customizes the HP for bosses, when Sonic hits the boss,
;   | it also takes place for customizing the variable used
;   | when fighting with bosses using custom HP.
...
DebugCheat = 0|CheatsEnabled
;	| Enabling this will enable the debug mode cheat.
LevSelCheat = 0|CheatsEnabled
;	| Level select cheat that allows you go in any zone.
SlowMotionCheat	= 0|CheatsEnabled
;	| Known cheat that slows down the gameplay, also enables
;	| that cheat without needing to type the Debug cheat.
CreditsCheat = 0|CheatsEnabled
;	| A cheat from REV that allows having ending sequence
;   | and skip in credits screen (Selbi).
...
ErrorTesting  = 0
;	| This will enable a test with intentional crash from
;   | MD Debugger for testing purpose.
...
; ===========================================================================
; Modifications for ROM building and addons
	include "Mods.asm"
...
; Simplifying macros and functions
    include "MegaPCM.Macros.asm"	; Mega PCM 2.1 Macros
	include	"Macros.asm"
...
.clearRAM:
		...
		if LevSelCheat=1
		move.b	#1,(f_levselcheat).w ; enable debug mode
		endif
		if SlowMotionCheat=1
		move.b	#1,(f_slomocheat).w ; enable debug mode
		endif
		if CreditsCheat=1
...
Art_Text:	binclude	"artunc/menutext.bin" ; text used in level select
Art_Text_End:	even

Art_DEBUGTEXT:	binclude	"artunc/debugtext.bin" ; duplicate for debug mode
Art_DEBUGTEXT_End:		even
...
Pal_GHZCyc2:	binclude	"palette/Cycle - GHZ 2.bin"
...
Pal_GHZ2:	bincludePalette	"palette/Green Hill Zone 2.bin"
...
.saveX:
		...
		addq.w	#2,d0		; add 2 px motion for the title screen
		endif
...
LevSel_SndTest:
		...
		bhs.s	LevSel_Right	; If not negative, go to Right check
		move.w	#$FF,d0		; If below 0, wrap around to 255 ($FF)

LevSel_Right:
		btst	#bitR,d1
		beq.s	LevSel_Refresh2
		addq.w	#1,d0		; Add 1
		cmpi.w	#$100,d0	; Is it 256?
		blo.s	LevSel_Refresh2	; If 0-255, it's fine; branch to refresh
		moveq	#0,d0		; If it reached 256, reset to 0
...
PlayCurrentActMusic:
		...
		bra.w	PlaySound_Special		; play current level music
...
Level_GetBgm:
		tst.w	(f_demo).w			; are we in a credits demo?
		bmi.s	Level_SkipTtlCard		; if yes, don't change music or load title card
...
GM_Special:
		...
		bsr.w	PlaySound_Special			; play correct special stage BG music
...
SS_ContinueSetup:
		...
		move.w	#$98,(v_ssrotate).w ; set stage rotation speed
		else
...
Map_SS_Down:	include	"_maps/SS DOWN Block.asm"
	...
	if CentisecondsHUD=1
Map_HUD:	include	"_maps/Centiseconds HUD.asm"
	else
...
Art_Hud:	binclude	"artunc/HUD Numbers.bin" ; 8x16 pixel numbers on HUD
		if	CentisecondsHUD=1
		binclude	"artunc/HUD Numbers Separators.bin" ; ' and " separators for the time
		even
		endif
		even
